...
connectBtn.addEventListener("click", async () => {
  if (connected) {
    ...
    userAddress = null;
    connected = false;
    walletText.textContent = "Disconnected";
  ...
  try {
    if (!window.ethereum) {
      walletText.textContent = "MetaMask not detected.";
      return;
    }
    ...
    if (chainId !== 137) {
      walletText.textContent = "Please switch to Polygon Mainnet (Chain ID 137)";
      return;
    }

    walletText.textContent = `Connected: ${userAddress}`;
    connectBtn.textContent = "🔌 Disconnect Wallet";
    connected = true;
  } catch (err) {
    walletText.textContent = `Connection error: ${err.message}`;
  }
});
